...
#
# vifa_launcher/transitions/plugins/slide.py
...
# ---------- Overlay that ONLY slides the two page images (background remains static) ----------
class _SlideOverlay(QWidget):
    def __init__(self, parent_stack: QStackedWidget, from_img: QImage, to_img: QImage,
        ...
        self._dir = 1 if direction >= 0 else -1   # +1: forward (right), -1: backward (left)
        self._ease = easing
        self._t = 0.0
        self.show()
        self.raise_()
    ...
    def paintEvent(self, _):
        ...
        # t=0:  from_x=0,         to_x= dir*W
        # t=1:  from_x=-dir*W,    to_x= 0
        from_x = int(-self._dir * tt * W)
        to_x   = int(self._dir * (1.0 - tt) * W)
        p = QPainter(self)
        ...
        p.drawImage(from_x, 0, self._from)
        p.drawImage(to_x,   0, self._to)
        p.end()

class SlideTransition(TransitionStrategy):
    name = "slide"
    ...
    def start(self, stack: QStackedWidget, to_index: int, duration_ms: int):
        ...
        overlay = _SlideOverlay(stack, from_img, to_img, direction, easing)
